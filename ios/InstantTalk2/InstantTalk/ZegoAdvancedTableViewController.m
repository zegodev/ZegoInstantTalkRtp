...
- (void)viewDidLoad {
    ...
    if ([ZegoInstantTalk appID] != 0) {
        [self.appID setText:[NSString stringWithFormat:@"%u", [ZegoInstantTalk appID]]];
    }
...
- (void)viewWillDisappear:(BOOL)animated {    
    if (self.appID.text.length > 0)
    {
        ...
        [ZegoInstantTalk setCustomAppID:(uint32_t)appID sign:self.appSign.text];
    }
    
    [ZegoInstantTalk setUsingTestEnv:self.testEnvSwitch.on];

...
/**
 ...
 @return 版本号
 @attention 开发者使用本 API 获取 SDK 版本号
 */
+ (NSString *)version;
...
/**
 ...
 @attention 建议在初始化 SDK 前调用
 @note 建议开发者在开发阶段设置为测试环境，使用由 Zego 提供的测试环境。上线前需切换为正式环境运营
 */
...
/**
 ...
 @note zegoliveroom 自带 IM 功能，随 SDK 初始化。如果要额外使用聊天室，需要启用聊天室功能
 */
...
/**
 ...
 @attention 确保在 loginRoom 前设置成功
 @note userID 和 userName 由业务方自己控制
 */
...
/**
 ...
 @return true 成功，false 失败
 @attention 使用 room 功能，初始化相关视图控制器时需要设置代理对象
...
/**
 ...
 @param block 消息发送结果
 @return true 成功，false 失败
 @attention 信令内容由用户自定义
 @note 发送成功后，结果通过 block 回调
 */
...
/**
 ...
 @return true 成功，false 失败
 @attention 设置代理对象成功后，在 [ZegoLiveEventDelegate -zego_onLiveEvent:info:] 中获取直播状态，状态参考 ZegoLiveEvent 定义
 @note 未设置代理对象，或对象设置错误，可能导致无法正常收到相关回调
 */
...
/**
 ...
 @return true 成功，false 失败
 @attention 开发者获取音视频设备错误，需要先设置此代理对象
 @note 未设置代理对象，或对象设置错误，可能导致无法正常收到相关回调
 */
...
/**
 ...
 @param roomID 房间 ID
 @attention 主播推流成功后调用 [ZegoLiveRoomApi (Publisher) -updateStreamExtraInfo:] 更新附加信息，在此回调中通知房间内其他成员
 @note 调用 [ZegoLiveRoomApi (Publisher) -updateStreamExtraInfo:] 更新信息的调用方，不会收到此回调
 */
...
/**
 ...
 @param content 消息内容
 @param roomID 房间 ID
 @attention 调用 [ZegoLiveRoomApi -sendCustomCommand:content:completion:] 发送自定义消息后，消息列表中的用户会收到此通知
 */
...
typedef enum : NSUInteger {
    Play_BeginRetry = 1,      /**< 播放直播开始重试 */
    Play_RetrySuccess = 2,    /**< 播放直播重试成功 */
    
    Publish_BeginRetry = 3,   /**< 发布直播开始重试 */
    Publish_RetrySuccess = 4, /**< 发布直播重试成功 */
} ZegoLiveEvent;
...
/**
 ...
 @param info 信息，目前为空
 @attention 调用 [ZegoLiveRoomApi -setLiveEventDelegate] 设置直播事件代理对象后，在此回调中获取直播事件状态
 */
...
/**
 ...
 @attention 调用 [ZegoLiveRoomApi -setDeviceEventDelegate] 设置设备事件代理对象后，在此回调中获取设备状态或错
 */
